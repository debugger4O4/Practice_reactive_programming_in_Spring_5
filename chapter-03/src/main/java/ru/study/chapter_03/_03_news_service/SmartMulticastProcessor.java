...
/**
 * Кэширует последний обзор. Поддерживает многоадресную рассылку (нет необходимости создавать один и тот же поток для
 * каждого подписчика Subscriber по отдельности). Включает механизм интеллектуального слежения за рассылкой и рассылает
 * новости только тем, кто прочитал предыдущий обзор
 */
